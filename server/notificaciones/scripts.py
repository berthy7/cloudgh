...
def insertions():
    with transaction() as session:
        ...
        if notificaciones_m is None:
            notificaciones_m = Modulo(title='Notificaciones', name='notificaciones', icon='notificacion.ico')

        correo_m = session.query(Modulo).filter(Modulo.name == 'correo').first()
        if correo_m is None:
            correo_m = Modulo(title='Envio de Correos ', route='/correo', name='correo', icon='correo.ico')

        correo_rrhh_m = session.query(Modulo).filter(Modulo.name == 'correo').first()
        if correo_rrhh_m is None:
            correo_rrhh_m = Modulo(title='Correos de RRHH ', route='/correo_rrhh', name='correo_rrhh', icon='correo.ico')
