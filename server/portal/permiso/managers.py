...
class PortalPermisoManager(SuperManager):
    ...
    def listar_todo(self):
        return self.db.query(self.entity).join(Tipoausencia).filter(self.entity.enabled == True).filter(
            Tipoausencia.tipo == "Permiso").order_by(self.entity.id.desc())



    def obtener_x_persona(self, fkpersona):
        return self.db.query(self.entity).join(Tipoausencia).filter(self.entity.enabled == True).filter(self.entity.fkpersona == fkpersona).filter(
            Tipoausencia.tipo == "Permiso").order_by(self.entity.id.desc()).all()


    def obtener_x_supervisor(self, fkpersona):
        solicitudes = self.db.query(self.entity).join(Tipoausencia).filter(self.entity.enabled == True).filter(or_(self.entity.fkautorizacion == fkpersona,self.entity.fkaprobacion == fkpersona)).filter(
            Tipoausencia.tipo == "Permiso").order_by(self.entity.id.desc()).all()
