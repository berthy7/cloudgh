...
class PortalAusenciaManager(SuperManager):
    ...
    def obtener_x_supervisor(self, fkpersona):
        return self.db.query(self.entity).filter(self.entity.fksuperior == fkpersona).all()
    ...
    def obtener_ausencia(self,fkpersona,fecha):
        ...
        if ajuste.oracle:
            # version oracle
            ausencia = self.db.query(Ausencia).filter(Ausencia.fkpersona == fkpersona).filter(
                Ausencia.estado == "Aceptado").filter(
                and_(func.to_date(Ausencia.fechai) <= fecha, func.to_date(Ausencia.fechaf) >= fecha)).all()
        else:
            # version postgres
            ausencia = self.db.query(Ausencia).filter(Ausencia.fkpersona == fkpersona).filter(
                Ausencia.estado == "Aceptado").filter(
                and_(func.date(Ausencia.fechai) <= fecha, func.date(Ausencia.fechaf) >= fecha)).all()
