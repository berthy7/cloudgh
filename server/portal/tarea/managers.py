...
from ...control.tarea.models import *
...
class PortalTareaManager(SuperManager):
    ...
    def list_all(self):
        ...
        if ajuste.oracle:
            ...
            return dict(objects=self.db.query(self.entity).filter(self.entity.enabled == True).filter(
                func.to_date(self.entity.fechaInicio).between(fechahoy, fechahoy)))
        else:
            # version postgres
            return dict(objects=self.db.query(self.entity).filter(self.entity.enabled == True).filter(
    ...
    def list_by_day(self):
        ...
        if ajuste.oracle:
            # version oracle
            return dict(tareas=self.db.query(self.entity).filter(func.to_date(self.entity.fechaInicio).between(fechahoy, fechahoy)).filter(self.entity.enabled == True).all())

        else:
            # version postgres
            return dict(tareas=self.db.query(self.entity).filter(func.date(self.entity.fechaInicio) == fechahoy).filter(self.entity.enabled == True).all())
    ...
    def obtener_tareas_dia(self):
        ...
        if ajuste.oracle:
            # version oracle
            return self.db.query(self.entity).filter(self.entity.enabled == True).filter(func.to_date(self.entity.fecha).between(fecha_dia.date(), fecha_dia.date())).all()

        else:
            # version postgres
            return self.db.query(self.entity).filter(self.entity.enabled == True).filter(func.date(self.entity.fecha) == fecha_dia.date()).all()
