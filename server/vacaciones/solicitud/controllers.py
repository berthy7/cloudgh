...
class V_solicitudController(CrudController):

    manager = V_solicitudManager
    html_index = "vacaciones/solicitud/views/index.html"
    html_table = "vacaciones/solicitud/views/table.html"
    routes = {
        '/v_solicitud': {'GET': 'index', 'POST': 'table'},
        '/v_solicitud_insert': {'POST': 'insert'},
        '/v_solicitud_update': {'PUT': 'edit', 'POST': 'update'},
        '/v_solicitud_delete': {'POST': 'delete'},
        '/v_solicitud_autorizacion': {'PUT': 'edit', 'POST': 'autorizacion'},
        '/v_solicitud_aprobacion': {'PUT': 'edit', 'POST': 'aprobacion'},
        '/v_solicitud_imprimir': {'POST': 'imprimir'},
        '/v_solicitud_reporte_xls': {'POST': 'imprimirxls'},

    }

    def get_extra_data(self):
        aux = super().get_extra_data()
        aux['tipovacacion'] = V_tipovacacionManager(self.db).listar_todo()
        ...
        aux['solicitudes'] = V_solicitudManager(self.db).listar_todo()
    ...
    def autorizacion(self):
        ...
        #
        #     if int(solicitud.fktipovacacion) == 3:
        #         for persona_colectiva in solicitud.colectiva:
        #             V_historicoManager(self.db).actualizar_vacaciones_solicitud(solicitud, persona_colectiva.fkpersona,
        #                                                                         diccionary['user'], diccionary['ip'])
        #             V_solicitudManager(self.db).actualizar_vacacion(solicitud, persona_colectiva.fkpersona)
        #
        #
        #     else:
        #         V_historicoManager(self.db).actualizar_vacaciones_solicitud(solicitud, solicitud.fkpersona,
        #                                                                     diccionary['user'], diccionary['ip'])
        #         V_solicitudManager(self.db).actualizar_vacacion(solicitud, solicitud.fkpersona)
    ...
    def imprimir_(self):
        ...
        html = "<meta http-equiv='Content-Type' content='text/html'; charset='utf-8' />" \
             ...
             "<div class='row'>" \
             "<div class='col-md-4'>" \
             "<h6 align='center'>INFORME SOLICITUD DE VACACIONES</h6>" \
                           "</div>" \
                           "</div>" \
                           "</div>"
        ...
        html += "<table>" \
                ...
                "<td colspan='4' class='text-normal' align='left'>"+ persona+"</td>" \
                "</tr>" \
                "<tr style='font-size: 12px'>" \
                "<th colspan='2' scope='colgroup'></th>" \
                "<th colspan='2' scope='colgroup' align='left'>Tipo de Vacacion: </th>" \
                "<td colspan='4' class='text-normal' align='left'>"+ solicitud.tipovacacion.nombre +"</td>" \
               "</tr>" \
               "<tr style='font-size: 12px'>" \
               "<th colspan='2' scope='colgroup'></th>" \
        ...
        nombre = "Solicitud_vacaciones.pdf"
    ...
    def imprimir(self):
        ...
        for det in ListadetalleVacacion:
            ...
            detalleVacacion += "<p>&nbsp;&nbsp;&nbsp;&nbsp;" +  str(det.gestion) + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+ str(det.dias) + str(espaciosBlancos)+ str(det.estado) +"</p>"
        ...
        dias_vacacion = BitacoraManager(self.db).obtener_cant_dias(solicitud.fktipovacacion,solicitud.fechai,solicitud.fechaf)
    ...
    def imprimirxls(self):
        ...
        cname = self.manager(self.db).persona_excel(diccionary)
