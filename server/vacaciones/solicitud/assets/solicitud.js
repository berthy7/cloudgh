    ...
    buttons: [
        {  extend : 'excelHtml5',
           exportOptions : { columns : [0, 1, 2, 3, 4, 5 ,6 ,7,8]},
            sheetName: 'Solicitud vacacion',
           title: 'Solicitud vacacion'  },
        {  extend : 'pdfHtml5',
...
$('#new').click(function () {
    ...
    $('#horai').parent().addClass('focused');
    $('#horaf').parent().addClass('focused');
    $('.hora_i_f').hide()
    document.getElementById("fechai").value=hoy
    // document.getElementById("fechaf").value=hoy
    $('#estadoautorizacion').val('Pendiente')
...
$('#insert').click(function () {
    ...
    if (notvalid === false) {
        ...
        ajax_call_post("v_personal_disponible", {
            ...
            //console.log(response)
            if (response.success === true) {
                ajax_call_post("v_solicitud_insert",{
                    _xsrf: getCookie("_xsrf"),
                    object: objeto_verificar
                }, function (response) {
                    //console.log('salida')
...
function attach_edit() {
    $('.edit').click(function () {
        ...
        ajax_call_get('v_solicitud_update', {
        _xsrf: getCookie("_xsrf"),
        object: obj
    }, function (response) {
            var self = response;
    ...
    $('.autorizacion').click(function () {
        ...
        ajax_call_get('v_solicitud_autorizacion', {
            _xsrf: getCookie("_xsrf"),
            object: obj
        }, function (response) {
            var self = response;
    ...
    $('.aprobacion').click(function () {
        ...
        ajax_call_get('v_solicitud_aprobacion', {
            _xsrf: getCookie("_xsrf"),
            object: obj
        }, function (response) {
            var self = response;
...
$('#autorizacion').click(function () {
    if (!validationInputSelects("form")) {
        ...
        ajax_call('v_solicitud_autorizacion', {
            object: objeto,
            _xsrf: getCookie("_xsrf")
        }, null, function () {
            setTimeout(function () {
        ...
        swal('Error de datos.', 'Hay campos vacíos por favor verifique sus datos.', 'error')
    }
})
...
$('#aprobacion').click(function () {
    if (!validationInputSelects("form")) {
        ...
        ajax_call('v_solicitud_aprobacion', {
            object: objeto,
            _xsrf: getCookie("_xsrf")
        }, null, function () {
            setTimeout(function () {
...
$('.delete').click(function () {
    ...
    swal({
        title: "¿Desea dar de baja los datos de la v_solicitud?",
        type: "warning",
...
function ajax_call_solicitud(url, data, render, callback) {
    $.ajax({
        ...
        //si exito es verdarero, se inserto v_solicitud
        if (exito === true || exito === "true"){
            var exito;

            exito = () => {
                swal('Operación exitosa.', 'Ausencia ingresada correctamente', 'success')
            }
            exito();
        } else if (response.indexOf('None') > -1) {
            swal('Error.', 'No tiene un superior asignado', 'error')
        }
